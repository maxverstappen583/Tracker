/* script.js — Polling Lanyard REST (no WebSocket)
   - Polls every POLL_MS
   - Shows status on first line and last-seen on second line (update every second when offline)
   - Spotify signature-based refresh: updates when song/artist/start/album-art changes
   - Progress bar min width for short songs, album color sampling for progress tint
*/

const USER_ID = "1319292111325106296"; // <-- put your Discord ID here
const POLL_MS = 4000;
...
const setImg = (id, src) => { const el = $(id); if (el && src) el.src = src; };

document.addEventListener("DOMContentLoaded", () => {
  // initial defensive UI unblock
...
async function run(){
  try {
    ...
    setImg("bannerImg", buildBanner(user));
    ...
    function startOfflineIntervalNow(){
      stopOfflineInterval();
      if (lastOnlineTimestamp) setText("lastSeen", `Last seen ${msToHumanShort(Date.now() - lastOnlineTimestamp)} ago`);
      else setText("lastSeen", "Last seen unknown");
    ...
    // Transition rules (show once on transition for online/idle/dnd, then hide while unchanged)
    if (status === "online" && lastStatus !== "online") {
      ...
      if (status === "offline") {
        if (!offlineInterval) startOfflineIntervalNow();
      } else {
        stopOfflineInterval();
      }
    }
...
/* simple badges (icons only when exist) */
function badgeDefs() {
  return [
...
/* ---------- Spotify rendering for polling ---------- */
async function renderSpotifyPolling(spotify) {
  // clear existing
  ...
  (async () => {
    ...
    const color = await sampleColor(albumArtUrl);
...
/* sample dominant-ish color from album art */
